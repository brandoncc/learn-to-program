...
puts 'How many bottles of beer are currently on the wall?'
bottles_left = gets.chomp.to_i

while bottles_left > 0
  bottles_text = bottles_left > 1 ? 'bottles' : 'bottle'
  puts english_number(bottles_left).capitalize + " #{bottles_text} of beer on the wall!"
  puts english_number(bottles_left).capitalize + " #{bottles_text} of beer!"
  puts 'Take one down, pass it around!'
  bottles_left -= 1
  if bottles_left > 0
    puts english_number(bottles_left).capitalize + " more bottle#{bottles_left > 1 ? 's' : ''} of beer on the wall!"
    puts ''
  else
